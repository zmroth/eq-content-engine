// EQ Login Encryption - DES-CBC with zero key/IV
// Matches EQEmu's eqcrypt_block implementation
// Using pure JavaScript DES implementation for compatibility
...
const E = [32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13,
...
function generateSubkeys(key: Buffer): number[][] {
...
function desBlock(block: Buffer, subkeys: number[][], encrypt: boolean): Buffer {
  ...
  for (let i = 0; i < 16; i++) {
    const newR = xor(l, feistel(r, keys[i]));
    l = r;
    r = newR;
  }
...
// DES-CBC with all-zero key and IV (matches EQEmu)
const ZERO_KEY = Buffer.alloc(8, 0);
const ZERO_IV = Buffer.alloc(8, 0);

/**
 * Encrypt/decrypt data using DES-CBC with zero key/IV (EQ login protocol)
 */
export function eqcryptBlock(data: Buffer, encrypt: boolean): Buffer {
...
/**
 * Encrypt login credentials for login packet
 */
export function encryptCredentials(username: string, password: string): Buffer {
  // Format: "username\0password\0" padded to 8-byte boundary
  const creds = Buffer.from(`${username}\0${password}\0`, 'utf8');
  return eqcryptBlock(creds, true);
}

/**
 * Decrypt login credentials from login packet
 */
export function decryptCredentials(data: Buffer): { username: string; password: string } {
  const decrypted = eqcryptBlock(data, false);
